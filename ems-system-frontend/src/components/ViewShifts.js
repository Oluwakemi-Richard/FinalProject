...
//       // Update shifts after successful check-in
//       setShifts(shifts.map(shift => shift.id === shiftId ? { ...shift, checked_in: true } : shift));
//     } catch (error) {
...
//       // Update shifts after successful check-out
//       setShifts(shifts.map(shift => shift.id === shiftId ? { ...shift, checked_out: true } : shift));
//     } catch (error) {
...
//     return !shift.checked_in && !shift.checked_out;
//   };
...
//   const fetchShifts = async () => {
//     try {
//       const endpoint = showPastShifts ? 'past_shifts' : 'upcoming_shifts';
...
//   const canCheckIn = (shift) => {
//     // Only allow check-in if the shift is not already checked in and there are no shifts checked out after it
//     return !shift.checked_in && !shifts.some(s => s.id !== shift.id && s.checked_out);
//   };

//   const canCheckOut = (shift) => {
//     // Only allow check-out if the shift is checked in and there are no shifts checked in after it
...
//     <div>
//       <h1>View Shifts</h1>
//       <Button variant="secondary" onClick={() => setShowPastShifts(!showPastShifts)}>
//         {showPastShifts ? 'View Upcoming Shifts' : 'View Past Shifts'}
//       </Button>
//       <Table striped bordered hover>
//         <thead>
//           <tr>
